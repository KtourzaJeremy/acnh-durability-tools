...
export default function CountersTool(props) {
  const { countersTools: counters, onHitCounterTool: onHit, resetCounterTool: reset, upgradeCounterTool: upgrade, upgradeCounterFlimsyTool:upgradeFlimsyAxe, removeCounterTool: remove } = useCountersTools()
  ...
  return (
    <div className="CountersTool">
      {counters.map(counter => {
        ...
        return (
          <div className={"CounterTool " + (almostBroken ? "almostBroken" : "")} key={counter.id}>
            <div className="CounterContent">
              <h3><FormattedMessage id={tool.id} /></h3>
              <img src={tool.image} alt={tool.id} />
              <div className="CounterTool--hits">
                {tool.hits > 1 && (
                  <button disabled={minuslimit} onClick={() => onHit(counter, (minushitslimit ? (counter.hit * (-1)) : (tool.hits * (-1))))}>
                ...
                {tool.hits > 1 && (
                  <button disabled={pluslimit} onClick={() => onHit(counter, (plushitslimit ? (tool.durability - counter.hit) : tool.hits))}>
                ...
                tool.upgrade && (
                  isFlimsyAxe
                    ...
                    <Popup className="Popup" 
                              trigger={<button><FormattedMessage 
                      ...
                      <Grid centered columns={2}>
                        <Grid.Row>
                          <Grid.Column textAlign='center'>
                            <h4><FormattedMessage id={tool.upgrade[0]}/></h4>
                            <Button className="btnUpgrade" onClick={() => upgradeFlimsyAxe(counter,tool.upgrade[0])}><img src={showImgTool(tool.upgrade[0])} alt={showImgTool(tool.upgrade[0])} /></Button>
                          </Grid.Column>
                          <Grid.Column textAlign='center'>
                            <h4><FormattedMessage id={tool.upgrade[1]}/></h4>
                            <Button className="btnUpgrade" onClick={() => upgradeFlimsyAxe(counter,tool.upgrade[0])}><img src={showImgTool(tool.upgrade[1])} alt={showImgTool(tool.upgrade[1])} /></Button>
                          </Grid.Column>
                        </Grid.Row>
                      </Grid>
                    </Popup>)
                    : (<button onClick={() => upgrade(counter)}><FormattedMessage id="INTERFACE.UPGRADE" /></button>)
                )
              }
              </div>
            </div>
